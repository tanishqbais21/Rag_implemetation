...
class DataIngestionPipeline:
    ...
    def _load_documents(self, directory_path: str) -> Tuple[list[Document], list[Path]]:
        """
        Load multiple document types (PDF, TXT, DOCX) from a directory.
        Only loads new or modified files.
        
        Args:
            directory_path (str): The path to the directory containing documents.
        
        Returns:
            Tuple[list[Document], list[Path]]: The loaded documents and list of processed file paths.
        """
        all_documents = []
        ...
        try:
            ...
            # File tracking state
            tracker_file = Path("data/.file_tracker.json")
            try:
                ...
                file_states = {}
                self.log.error(f"Error loading file states: {e}")
            ...
            for file_path in all_files:
                ...
                if ext in loaders_mapping:
                    try:
                        ...
                        if file_str in file_states and file_states[file_str] == mtime:
                ...
                # 4. ignore unsupported files
                elif ext: 
                    self.log.warning(f"Skipping unsupported file type {ext}: {file_path.name}")

            self.log.info(f"Successfully loaded a total of {len(all_documents)} new/modified document parts/pages.")
            return all_documents, processed_file_paths
    ...
    def run_pipeline(self, file_path: str)->None:
        ...
        try:
            documents, processed_file_paths = self._load_documents(file_path)
            if not documents:
                self.log.info("No new or modified files found. Skipping data ingestion pipeline.")
                return
            ...
            # Update tracker file with modified timestamps
            tracker_file = Path("data/.file_tracker.json")
            try:
                if tracker_file.exists():
                    with open(tracker_file, "r") as f:
                ...
                for p in processed_file_paths:
                    file_states[str(p)] = os.path.getmtime(p)
                ...
                self.log.error(f"Error saving file tracker state: {tracker_err}")
