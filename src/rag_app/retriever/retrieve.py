...
import os
class RAGRetriever:
    def __init__(self,vector_store:vector_db.VectorStore,ingestion_object:load_data.DataIngestionPipeline):
        self.log=AILogger(name="RagRetriever",log_dir="logs/RagRetriever").logger
        self.vector_store=vector_store
        self.ingestion_object=ingestion_object
        
    def retrieve(self,query:str,top_k:int=5,score_threshold:float=0.0)->list[Dict[str,Any]]:
        try:
            # Generate embeddings for the query
            self.log.info(f"Generating embeddings for query: {query}"   )
            query_embedding = self.ingestion_object.generate_embeddings([query])
    ...
    def get_answer(self,query:str)->str:
        ...
        #response = self.ingestion_object.llm.invoke(prompt)
        #self.log.info(f"Generated prompt for query: {prompt}")
        return prompt